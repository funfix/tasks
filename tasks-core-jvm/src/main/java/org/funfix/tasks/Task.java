...
public abstract class Task<T> implements Serializable {
     ...
     */
    abstract Cancellable executeAsync(CompletionCallback<? super T> listener);
    ...
    final T executeBlocking() throws ExecutionException, InterruptedException {
        final var h = new BlockingCompletionCallback<T>();
    ...
    public static <T> Task<T> create(
        final Function<CompletionCallback<? super T>, Cancellable> builder
    ) {
    ...
    public static <T> Task<T> successful(final T value) {
        return new Task<>() {
            @Override
            Cancellable executeAsync(final CompletionCallback<? super T> listener) {
...
final class CreateAsyncTask<T> extends Task<T> {
    private final Function<CompletionCallback<? super T>, Cancellable> builder;
    ...
    Cancellable executeAsync(final CompletionCallback<? super T> listener) {
        ...
        final Runnable run = () -> {
            try {
                final var token = builder.apply(CompletionCallback.protect(listener));
                cancel.set(token);
...
final class TaskFiber<T> implements Fiber<T> {
    ...
    public final CompletionCallback<T> onComplete = new CompletionCallback<>() {
        @Override
        public void onSuccess(final T value) {
...
final class TaskFromThreadFactory<T> extends Task<T> {
    ...
    private final class TaskRunnable implements Runnable {
        private final CompletionCallback<? super T> listener;

        public TaskRunnable(final CompletionCallback<? super T> listener) {
...
final class TaskFromExecutor<T> extends Task<T> {
    ...
    @Override
    public Cancellable executeAsync(final CompletionCallback<? super T> onComplete) {
...
final class TaskFromCompletionStage<T> extends Task<T> {
    ...
    @Override
    public Cancellable executeAsync(final CompletionCallback<? super T> listener) {
        var userError = true;
        try {
